...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <!-- Maintenance note: For each revision, change the content of &lt;recordInfo&gt;&lt;recordOrigin&gt; to reflect the new revision number.
            ...
            Revision 2.07 - used substring-before function to get marc:subfield b (ie., publisher) and marc:subfield c (i.e., dateIssued). cm3 2022/12/08
    ...
    <xsl:template match="/">
        <xsl:result-document encoding="UTF-8" version="1.0" method="xml" media-type="text/xml"
            ...
            <xsl:choose>
                ...
                <xsl:when test="//marc:record and not(//marc:collection)">
...
-->
    ...
    -->
            ...
            <xsl:for-each
                select="marc:datafield[@tag = 033][@ind1 = '2']/marc:subfield[@code = 'a'][1]">
                <dateCaptured encoding="iso8601" point="start">
                    <xsl:value-of select="."/>
                </dateCaptured>
            </xsl:for-each>
            <xsl:for-each
                select="marc:datafield[@tag = 033][@ind1 = '2']/marc:subfield[@code = 'a'][2]">
        ...
        <xsl:for-each select="marc:datafield[@tag = 264][@ind2 = 1]">
            <originInfo displayLabel="publisher">
                <!-- Revision 2.07 used substring-before function to get marc:subfield b (ie., publisher) and marc:subfield c (i.e., dateIssued) 2022/12/08 -->
...
-->
        ...
        <xsl:if test="marc:datafield[@tag = 070][@ind1 = '0']/marc:subfield[@code = 'a']">
            <classification authority="nal">
                <xsl:value-of
                    select="marc:datafield[@tag = 070][@ind1 = '0']/marc:subfield[@code = 'a']"/>
        ...
        <xsl:if test="marc:datafield[@tag = 024][@ind1 = '7']">
            <identifier type="doi">
                <xsl:value-of
                    select="marc:datafield[@tag = 024][@ind1 = '7']/marc:subfield[@code = 'a']"/>
            </identifier>
        </xsl:if>
...
-->
    ...
    <xsl:template name="createGenreFrom336">
        <genre>
            ...
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <!-- Revision 2.05 removed marc:subfield "b" from appeearing in hyphenated format-->
            <xsl:call-template name="subfieldSelect">
                <xsl:with-param name="codes">a</xsl:with-param>
            </xsl:call-template>
        </genre>
