...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="/">  
        ...
        <xsl:variable name="Monthyear" select="format-date(current-date(), '[MN,*-3][Y01]')"/>
        <xsl:choose>
            <xsl:when test="starts-with($fileName, 'IND')">
                <xsl:result-document exclude-result-prefixes="xd xlink xs xsi" 
                    ...
                    format="combine" href="{$workingDir}recompile/N-agricola_IND_{$Monthyear}_{$fileNumber}.mods.xml"> 
