...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xd:doc scope="stylesheet">
        ...
        <xd:param name="fileNumber">
            <xd:p>Tokenizes forward slashes found within the base-uri()'s file/directory path structure. Upon reaching the last forward slash, the expath processes the instruction subract 4 directories backwards. 
                Uses the resplace fuction to select only the file number from the whole file name to accuractetly name the output file.</xd:p>
        </xd:param>
        ...
        <xsl:choose>
            <xsl:when test="starts-with($fileName, 'IND')">
                <xsl:result-document exclude-result-prefixes="xd xlink xs xsi" 
                    ...
                    format="combine" href="{$workingDir}/recompile/agricola_IND_Mar23_{$fileNumber}.mods.xml"> 
                    <modsCollection xmlns="http://www.loc.gov/mods/v3"
                        xmlns:mods="http://www.loc.gov/mods/v3"
                        xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                        xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-7.xsd">
                        <xsl:copy-of copy-namespaces="no" select="document(collection/doc/@href)/mods:collection/mods:mods"/>
                    </modsCollection>
                </xsl:result-document>
            </xsl:when>
            <xsl:otherwise>
