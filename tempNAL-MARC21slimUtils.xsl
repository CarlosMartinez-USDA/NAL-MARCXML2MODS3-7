...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    xmlns:local="http://wwww.local.gov/namespace" xmlns:marc="http://www.loc.gov/MARC21/slim"
    ...
    <xd:doc scope="stylesheet" id="Institution-id-LC">
        <xd:desc>
            <xd:ul>
                <xd:li>
                    <xd:li><xd:p><xd:b>XLST Function Library</xd:b></xd:p></xd:li>
                    <xd:ul>
                        ...
                        <xd:li>
                            <xd:p><xd:b>Modified on:</xd:b>February 16, 2023</xd:p>
                        </xd:li>
                        <xd:li>
                            <xd:p><xd:b>Author:</xd:b>Carlos Martinez III</xd:p>
                        </xd:li>
                        <xd:li>
                            <xd:p><xd:b>Modified on:</xd:b>May 22, 2023</xd:p>
                        </xd:li>
                    </xd:ul>
                    <xd:ul>
    ...
    <!-- XSLT 1.0 template utilites for MARC21 -->
    <!-- datafield -->
    <xd:doc>
    ...
    <xsl:function name="local:convertDataType">
        <xsl:param name="datafield" as="node()"/>
        <xsl:param name="codes"/>
        <!-- codeStr -->
        <xsl:variable name="codeStr">
            <xsl:choose>
                <xsl:when test="$codes castable as xs:integer">
                    <xsl:value-of select="string($codes)"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <!-- Selects and prints out datafield -->
        <xsl:variable name="str">
            <xsl:for-each select="$datafield/child::*[contains($codeStr,@code)]">
                <xsl:value-of select="concat(.,' ')"/>
            </xsl:for-each>
        </xsl:variable>
        <xsl:value-of select="substring($str,1,string-length($str)-1)"/>
...
-->
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose:</xd:b>The f:sentence-case function treats a string like two parts;
                the first character of $arg is capitalized, the rest of the string is lowercase.
                (e.g. "STRING THEORY" ---> "String theory"). Thus, even if the second letter of $arg
                is not a lowercase letter, the rest of the string is still lowercase. </xd:p>
        </xd:desc>
        <xd:param name="arg"/>
    </xd:doc>
    ...
    <xsl:function name="f:proper-case">
        <xsl:param name="arg" as="xs:string?"/>
        <xsl:variable name="otherChars" as="item()*"
            select="f:substring-before-match($arg, '\-|[A-Z].[A-Z].|\s')"/>
        ...
        <xsl:sequence select="
                ...
                then
                    concat(f:sentence-case($substring-1a), ($otherChars), (f:sentence-case($substring-1b)))
                else
                    if (contains($arg, $white-space))
                    then
                        concat(f:sentence-case($substring-2a), ($white-space), (f:sentence-case($substring-2b)))
                    else
                        f:sentence-case($arg)"/>
    </xsl:function>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>The f:nameIdentifier takes the xpath provided as $arg. First
                fn:matches($arg, $regex) tests the $arg. If $arg matches the $regex, then
    ...
    <xsl:function name="f:nameIdentifier" as="xs:string">
        ...
        <xsl:sequence select="
                ...
                then
                    ...
                    if ((contains($args, 'orcid') or contains($args, 'viaf') or contains($args, 'isni') or matches($args, '[a-z]+')) = true())
                    then
                        replace($args, '(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)', '$3')
                    else
                        $args"/>
        <!--  </xsl:variable>-->
        <!--  <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Returns: </xd:b>True or False</xd:p>
        </xd:desc>
        <xd:param name="value"/>
    </xd:doc>
