...
<!-- datafield -->
        ...
        <xsl:variable name="str">
            <xsl:choose>                
                ...
                <xsl:when test="marc:subfield">
                    ...
                    <xsl:if test="matches($codes, @code)">
    ...
    <xsl:template name="chopPunctuation">
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches($punctuation, substring($chopString, $length, 1))">
    ...
    <xsl:template name="chopPunctuationFront">
        ...
        <xsl:choose>
            <xsl:when test="$length = 0"/>
            <xsl:when test="matches('.:,;/[ ', substring($chopString, 1, 1))">
    ...
    <xsl:template name="url-encode">
        ...
        <xsl:if test="$str">
            ...
            <xsl:choose>
                <xsl:when test="matches($safe, $first-char)">
                    <xsl:value-of select="$first-char"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:variable name="codepoint">
                        <xsl:choose>
                            <xsl:when test="matches($ascii, $first-char)">
                                <xsl:value-of
                                    select="string-length(substring-before($ascii, $first-char)) + 32"
                                />
                            </xsl:when>
                            <xsl:when test="matches($latin1, $first-char)">
                                <xsl:value-of
                                    select="string-length(substring-before($latin1, $first-char)) + 160"/>
                                <!-- was 160 -->
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:message terminate="no">Warning: string matches a character
...
-->
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>The f:proper-case function performs a "choose when" test
                first determining if a string matches two substrings, when it does, it capitalizes
   ...
   <!-- <!-\- f:nameIdentifier -\->
    <xd:doc>
        ...
        <xsl:function name="f:nameIdentifier">
               ...
               <xsl:for-each select="$args">
                <xsl:variable name="arg">
                    <xsl:sequence select="
                        ...
                        then ('lcnaf')
                        else if ((matches($args, 'orcid') or matches($args, 'viaf') or  matches($args, 'isni') or  matches($args, '[a-z]+')) = true())
                        then replace($args, '(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)', '$3')
                        else $args"/>
                </xsl:variable>
                <xsl:choose>
                    <xsl:when test="matches($arg, 'orcid')">
                        <xsl:value-of select="'orcid'"/>
                    </xsl:when>
                    <xsl:when test="matches($arg, 'id.loc')">
                        <xsl:value-of select="'lcnaf'"/>
                    </xsl:when>
                    <xsl:when test="matches($arg, 'viaf.org')">
                        <xsl:value-of select="'viaf.org'"/>
                    </xsl:when>
                    <xsl:when test="matches($arg, 'nlm')">
                        <xsl:value-of select="'mesh'"/>
                    </xsl:when>
                    <xsl:when test="matches($arg, 'nal')">
